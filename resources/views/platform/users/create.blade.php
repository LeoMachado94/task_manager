                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6 form-group">
                                                ...
                                                <select type="text" id="level_access" class="select2 form-control" name="level_access">
                                                    <option value="{{ \App\Models\User::$ROLE_COLLABORATOR }}" @if(old('level_access') === \App\Models\User::$ROLE_COLLABORATOR) selected @endif>Collaborator</option>
                                                    <option value="{{ \App\Models\User::$ROLE_ADMINISTRATOR }}" @if(old('level_access') === \App\Models\User::$ROLE_ADMINISTRATOR) selected @endif>Administrator</option>
                                                    <option value="{{ \App\Models\User::$ROLE_SUPER_ADMINISTRATOR }}" @if(old('level_access') === \App\Models\User::$ROLE_SUPER_ADMINISTRATOR) selected @endif>Super Administrator</option>
                                                </select>
                                                @error('status')
                                                <span class="text-danger small">{{ $message }}</span>
                                                @enderror
